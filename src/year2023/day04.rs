...
// Card 6: 31 18 13 56 72 | 74 77 10 23 35 67 36 11";
    ...
    for card_str in input.trim().split('\n') {
        ...
        let card = NumCard{ num_cards: 1, win_nums: win_nums, my_nums: my_nums };
        cards.push(card);
    }
    ...
    for i in 0..cards.len() {
        let card = &cards[i];
        let mut matches = 0;
        let num_copies = card.num_cards.clone();
        ...
        if matches > 0 {
            for extra in 1..=matches {
                let card_to_change = &mut cards[i + extra];
                card_to_change.num_cards += num_copies;
            }
        }
    }
